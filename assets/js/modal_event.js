$('#bsModal').on('show.bs.modal', function(event) {
    ...
    } else if (type === 'file' && edit !== undefined)  {
        modal.find('.modal-title').text(`Filemanager: edit ${edit}`);
        modal.find('iframe').attr("src", `/lib/tinyfilemanager/tinyfilemanager.php?p=${path}&edit=${edit}&env=ace`);
        modal.find('iframe').attr("style", "display:block");
    } else if (type === 'dir') {
...
$('#bsModal').on('shown.bs.modal', function() {
    ...
    if (modal.find('.modal-title').text() === 'ERROR') {
        ...
        // $('#bsModal').find('iframe').css(".text-muted", "display:none");
